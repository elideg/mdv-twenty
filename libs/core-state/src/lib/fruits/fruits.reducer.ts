...
export const fruitsAdapter: EntityAdapter<Fruit> = createEntityAdapter<Fruit>();
...
const fruitsReducer = createReducer(
  initialState,
  on(fruitsActions.fruitSelected, (state, { selectedFruitId }) =>
    Object.assign({}, state, { selectedFruitId })
  ),
  on(fruitsActions.fruitsLoaded, (state, { fruits }) =>
    fruitsAdapter.addAll(fruits, { ...state, isLoading: false })
  ),
  ...
  on(fruitsActions.fruitDeleted, (state, { fruit }) =>
    fruitsAdapter.removeOne(fruit.id, { ...state, isLoading: false })
  ),
  on(
